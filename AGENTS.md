...
This runs:
```
./gradlew assembleProductionVanillaOpenSourceDebug --no-daemon --max-workers=2 -Dorg.gradle.jvmargs='-Xmx4g ...'
```
...
- **Output APK**: `app/build/outputs/apk/productionVanillaOpenSource/debug/ProtonVPN-*.apk`

## Variant
`productionVanillaOpenSourceDebug` — production environment, vanilla functionality (no Google services), open-source distribution, debug build type.
